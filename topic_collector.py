"""臨床統計ラボ - トピック自動収集モジュール

NEWS_SOURCESからRSS + スクレイピングで最新記事を収集し、
Gemini APIで臨床統計関連のネタだけフィルタリング＆ランク付けする。
"""
import json
import logging
import re
...
class TopicCollector:
    ...
    def _filter_and_rank(self, items: list[dict]) -> list[dict]:
        """Gemini APIで臨床統計関連のネタをフィルタリング＆ランク付けする"""
...
臨床統計・生物統計・SASプログラミング・臨床試験に関連するものだけを抽出し、
...
【評価基準】
- 臨床統計・生物統計に直接関連する記事: スコア80-100
- SASプログラミング・R統計に関連する記事: スコア70-89
- 臨床試験全般に関連する記事: スコア50-69
- 関連性が低い記事: 除外
...
```json
[
  {{"index": 1, "score": 95, "category": "SAS実践テクニック", "blog_title_suggestion": "ブログ記事タイトル案", "keyword": "SEOキーワード"}}
]
```
        ...
        try:
            ...
            for r in ranked:
                ...
                if 0 <= idx < len(items):
                    ...
                    item["category"] = r.get("category", "臨床統計最新ニュース")
